...
function runOnBrowser(url, browserConfig, opts) {
    ...
    log('Num of repeats:', opts.repeat);
		return runRepeatTests(opts.repeat--, []);
	}).fin(function() {
	...
	function runRepeatTests(repeat, results) {
    log('Repeating :', repeat);
		return runTests().then(function(data) {
			results.push(data);
			if (repeat-- > 0) {
				return browser.refresh().then(function(){
					return runRepeatTests(repeat, results);
				});
			} else {
				return Q(results);
			}
